...
<body class="font-display bg-background-light text-text-light">
    ...
    <script>
        // Data karyawan dengan field divisi
        const karyawanData = [
            { id: 1, nama: "Ahmad Fauzi", jabatan: "Manager", divisi: "IT", gaji: 15000000, alamat: "Jl. Sudirman No. 123, Jakarta", kontak: "081234567890", foto: "" },
            { id: 2, nama: "Siti Nurhaliza", jabatan: "Staff", divisi: "HR", gaji: 8000000, alamat: "Jl. Thamrin No. 456, Jakarta", kontak: "082345678901", foto: "" },
            { id: 3, nama: "Budi Santoso", jabatan: "Staff", divisi: "Finance", gaji: 7500000, alamat: "Jl. Gatot Subroto No. 789, Jakarta", kontak: "083456789012", foto: "" },
            { id: 4, nama: "Dewi Lestari", jabatan: "Intern", divisi: "Marketing", gaji: 3000000, alamat: "Jl. Rasuna Said No. 321, Jakarta", kontak: "084567890123", foto: "" },
            { id: 5, nama: "Rudi Hermawan", jabatan: "Manager", divisi: "Finance", gaji: 16000000, alamat: "Jl. MH Thamrin No. 654, Jakarta", kontak: "085678901234", foto: "" },
            { id: 6, nama: "Andi Wijaya", jabatan: "Staff", divisi: "IT", gaji: 8500000, alamat: "Jl. Kemang No. 987, Jakarta", kontak: "086789012345", foto: "" },
            { id: 7, nama: "Maya Putri", jabatan: "Intern", divisi: "HR", gaji: 3500000, alamat: "Jl. Senayan No. 159, Jakarta", kontak: "087890123456", foto: "" },
            { id: 8, nama: "Eko Prasetyo", jabatan: "Staff", divisi: "Marketing", gaji: 9000000, alamat: "Jl. Sudirman No. 753, Jakarta", kontak: "088901234567", foto: "" },
            { id: 9, nama: "Lina Sari", jabatan: "Manager", divisi: "HR", gaji: 17000000, alamat: "Jl. Gatot Subroto No. 357, Jakarta", kontak: "089012345678", foto: "" },
            { id: 10, nama: "Fajar Nugroho", jabatan: "Intern", divisi: "IT", gaji: 4000000, alamat: "Jl. Rasuna Said No. 951, Jakarta", kontak: "090123456789", foto: "" }
        ];
        ...
        function updateVisibleItems() {
            ...
            karyawanRows.forEach((row, index) => {
            ...
            karyawanCards.forEach((card, index) => {
                if (index >= startIndex && index < endIndex) {
                    card.style.display = '';
                }
            });
        ...
        function initializeFilter() {
            ...
            applyFilterBtn.addEventListener('click', function() {
                ...
                applyFilters();
                filterDropdown.classList.remove('show');
                showMinimalPopup('Filter Diterapkan', `Menampilkan ${filteredData.length} karyawan`, 'success');
            });
        ...
        function applyFilters() {
            ...
            // Apply filters
            filteredData = karyawanData.filter(item => {
                // Check if jabatan matches filter
                let jabatanMatches = false;
                if (activeFilters.includes('all')) {
                    jabatanMatches = true;
                } else {
                    jabatanMatches = activeFilters.some(filter => item.jabatan.toLowerCase().includes(filter.toLowerCase()));
                }
                ...
                return jabatanMatches && searchMatches;
            });
        ...
        function renderKaryawanTable() {
            ...
            filteredData.forEach((item, index) => {
                ...
                row.className = 'karyawan-row';
                row.setAttribute('data-id', item.id);
                row.setAttribute('data-nama', item.nama);
                row.setAttribute('data-jabatan', item.jabatan);
                row.setAttribute('data-divisi', item.divisi);
                row.setAttribute('data-gaji', item.gaji);
                row.setAttribute('data-alamat', item.alamat);
                row.setAttribute('data-kontak', item.kontak);
                row.setAttribute('data-foto', item.foto || '');
                
                let statusBadge = '';
                switch(item.jabatan.toLowerCase()) {
                    case 'manager':
                        statusBadge = '<span class="status-badge status-manager">Manager</span>';
                        break;
                    case 'staff':
                        statusBadge = '<span class="status-badge status-staff">Staff</span>';
                        break;
                    case 'intern':
                        statusBadge = '<span class="status-badge status-intern">Intern</span>';
                        break;
                }
                ...
                row.innerHTML = `
                    ...
                    <td style="min-width: 150px;">${statusBadge}</td>
                    <td style="min-width: 150px;">${item.divisi}</td>
                    <td style="min-width: 150px;">Rp. ${item.gaji.toLocaleString('id-ID')}</td>
                    <td style="min-width: 250px;">${item.alamat}</td>
                    <td style="min-width: 150px;">${item.kontak}</td>
                    <td style="min-width: 100px;">
                        ${item.foto ? 
                            `<img src="${item.foto}" alt="${item.nama}" class="h-10 w-10 rounded-full object-cover">` :
                    ...
                    <td style="min-width: 100px; text-align: center;">
                        <div class="flex justify-center gap-2">
                            <button class="edit-btn p-1 rounded-full hover:bg-primary/20 text-gray-700" 
                                    data-karyawan='${JSON.stringify(item)}'>
        ...
        // Render karyawan cards for mobile
        function renderKaryawanCards() {
            const mobileCards = document.getElementById('mobile-cards');
            mobileCards.innerHTML = '';
            
            filteredData.forEach((item, index) => {
                const card = document.createElement('div');
                card.className = 'bg-white rounded-lg border border-border-light p-4 shadow-sm karyawan-card';
                card.setAttribute('data-id', item.id);
                card.setAttribute('data-nama', item.nama);
                card.setAttribute('data-jabatan', item.jabatan);
                card.setAttribute('data-divisi', item.divisi);
                card.setAttribute('data-gaji', item.gaji);
                card.setAttribute('data-alamat', item.alamat);
                card.setAttribute('data-kontak', item.kontak);
                card.setAttribute('data-foto', item.foto || '');
                ...
                card.innerHTML = `
                    <div class="flex justify-between items-start mb-3">
                        <div class="flex items-center gap-3">
                            ${item.foto ? 
                                `<img src="${item.foto}" alt="${item.nama}" class="h-12 w-12 rounded-full object-cover">` :
                    ...
                    <div class="grid grid-cols-2 gap-2 text-sm">
                        ...
                        <div>
                            ...
                            <p class="font-medium">Rp. ${item.gaji.toLocaleString('id-ID')}</p>
                        </div>
            ...
            // Add event listeners to edit and delete buttons
            document.querySelectorAll('.mobile-cards .edit-btn').forEach(button => {
                button.addEventListener('click', function() {
                    const data = JSON.parse(this.getAttribute('data-karyawan'));
                    openEditModal(data);
                });
            ...
            document.querySelectorAll('.mobile-cards .delete-btn').forEach(button => {
                button.addEventListener('click', function() {
                    const id = this.getAttribute('data-id');
                    openDeleteModal(id);
                });
        ...
        function initializeModals() {
            ...
            editKaryawanForm.addEventListener('submit', function(e) {
                ...
                if (index !== -1) {
                    karyawanData[index] = {
                        ...
                        gaji: parseInt(formData.get('gaji')),
                        alamat: formData.get('alamat'),
                        kontak: formData.get('kontak'),
                        foto: karyawanData[index].foto // Keep existing photo
                    };
            ...
            deleteKaryawanForm.addEventListener('submit', function(e) {
                e.preventDefault();
                
                // Get karyawan ID
                const id = parseInt(document.getElementById('deleteId').value);
                
                // Find and remove karyawan
                const index = karyawanData.findIndex(k => k.id === id);
                if (index !== -1) {
        ...
        function openEditModal(data) {
            ...
            if (data.foto) {
                document.getElementById('editFotoPreview').innerHTML = `<img src="${data.foto}" alt="${data.nama}" class="h-16 w-16 rounded-full object-cover">`;
            } else {
        ...
        function openDeleteModal(id) {
            ...
            document.getElementById('deleteKaryawanModal').classList.remove('hidden');
        }

        function closeDeleteModal() {
            document.getElementById('deleteKaryawanModal').classList.add('hidden');
        }
