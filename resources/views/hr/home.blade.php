...
<body class="font-display bg-background-light text-text-light">
    ...
    async function loadEmployeeStats() {
        try {
            // Load total karyawan
            const karyawanResponse = await fetch('/api/karyawan/count', {
                headers: {
            ...
            const totalEmployees = karyawanData.count || karyawanData.total || 0;
            document.getElementById('totalEmployees').textContent = totalEmployees;

            // Load active employees
            const activeResponse = await fetch('/api/karyawan/active', {
                headers: {
                    'Accept': 'application/json',
                    'X-CSRF-TOKEN': window.csrfToken
                }
            ...
            const activeEmployees = activeData.count || activeData.total || 0;
            document.getElementById('activeEmployees').textContent = activeEmployees;

            // Load divisi count
            const divisiResponse = await fetch('/divisis/list', {
                headers: {
                    'Accept': 'application/json',
                    'X-CSRF-TOKEN': window.csrfToken
                }
            ...
            const divisiArray = Array.isArray(divisiData) ? divisiData : (divisiData.data || []);
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        // Skip API calls that require general_manager role - they will show placeholder instead
        // These are disabled due to role restrictions:
