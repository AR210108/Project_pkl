...
<body class="font-display bg-background-light text-text-light">
                ...
                <form id="buatKwitansiForm" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <input type="number" id="subTotal" name="sub_total"
                                class="w-full px-3 py-2 bg-gray-100 border border-gray-300 rounded-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary"
                                required>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <input type="text" id="noRekening" name="no_rekening"
                            class="w-full px-3 py-2 bg-gray-100 border border-gray-300 rounded-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary"
                            value="4030-01-011093-53-6">
                    </div>
            ...
            <div class="p-6">
                ...
                <form id="editKwitansiForm" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Sub Total (Rp)</label>
                            <input type="number" id="editSubTotal" name="sub_total"
                                class="w-full px-3 py-2 bg-gray-100 border border-gray-300 rounded-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary"
                                required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Fee Maintenance (Rp)</label>
                            <input type="number" id="editFeeMaintenance" name="fee_maintenance"
                                class="w-full px-3 py-2 bg-gray-100 border border-gray-300 rounded-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary"
                                required>
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Total (Rp)</label>
                            <input type="text" id="editTotal" name="total_display"
                                class="w-full px-3 py-2 bg-gray-100 border border-gray-300 rounded-lg text-gray-700"
                                readonly>
                            <!-- Hidden input for the actual numeric total value -->
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">No. Rekening</label>
                        <input type="text" id="editNoRekening" name="no_rekening"
                            class="w-full px-3 py-2 bg-gray-100 border border-gray-300 rounded-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary">
                    </div>
                    <div class="flex justify-end gap-2 mt-6">
                ...
                <form id="deleteKwitansiForm" class="space-y-4">
                    <div class="mb-6">
                        <p class="text-gray-700 mb-2">Apakah Anda yakin ingin menghapus kwitansi untuk <span
                                id="deleteKwitansiNama" class="font-semibold"></span> dengan nomor order <span
                                id="deleteKwitansiNomor" class="font-semibold"></span>?</p>
        ...
        <div class="print-container">
            ...
            <div class="kwitansi-info">
                ...
                <div class="info-row">
                    ...
                    <div class="info-value">${kwitansi.nomor_order}</div>
                </div>
                <div class="info-row">
    ...
    `;
                        ...
                        fetch(`/admin/api/invoice-for-kwitansi/${invoiceId}`, {
                            ...
                            .then(data => {
                                if (data.success) {
                                    ...
                                    document.getElementById('namaPerusahaan').value = data.data
                                        .company_name || '';
                                    document.getElementById('nomorOrder').value = data.data
                                        .order_number || data.data.invoice_no || '';
                                    document.getElementById('namaKlien').value = data.data
                                        .client_name || '';
                                    document.getElementById('deskripsi').value = data.data
                                        .description || data.data.nama_layanan || '';
                                    document.getElementById('harga').value = data.data.total || 0;
                                    document.getElementById('subTotal').value = data.data.total || 0;
                                    document.getElementById('feeMaintenance').value = data.data
                                        .fee_maintenance || 0;

                                    // Calculate total
                                    calculateTotal();

                                    // Set status based on invoice payment status
                                    const statusSelect = document.getElementById('status');
                                    if (data.data.status_pembayaran === 'lunas') {
                                        statusSelect.value = 'Lunas';
                                    } else {
                                        statusSelect.value = 'Pembayaran Awal';
                                    }
                                } else {
                            ...
                            .catch(error => {
                                ...
                                showMinimalPopup('Error',
                                    'Terjadi kesalahan saat mengambil data invoice', 'error');
                            });
                    }
                });
            }
            ...
            pilihInvoice.addEventListener('change', function() {
                ...
                if (invoiceId) {
                    ...
                    fetch(`/admin/api/invoice-for-kwitansi/${invoiceId}`, {
                        ...
                        .then(data => {
                            console.log('Invoice detail received:', data); // Debug log

                            if (data.success) {
                                // Populate form dengan data invoice
                                document.getElementById('namaPerusahaan').value = data.data.company_name || '';
                                document.getElementById('nomorOrder').value = data.data.order_number || data
                                    .data.invoice_no || '';
                                document.getElementById('namaKlien').value = data.data.client_name || '';
                                ...
                                document.getElementById('harga').value = data.data.total || 0;
                                document.getElementById('subTotal').value = data.data.total || 0;
                                document.getElementById('feeMaintenance').value = 0; // Default

                                // Hitung total
                                calculateTotal();

                                // Set status berdasarkan invoice payment status
                                const statusSelect = document.getElementById('status');
                                if (data.data.status_pembayaran === 'lunas') {
                                    statusSelect.value = 'Lunas';
                                } else {
                    ...
                    if (data.success && data.data && data.data.length > 0) {
                        ...
                        document.querySelectorAll('.edit-kwitansi-btn').forEach(btn => {
                            btn.addEventListener('click', function() {
                                ...
                                const bank = this.getAttribute('data-bank') || 'BRI';
                                const noRekening = this.getAttribute('data-no-rekening') ||
                                    '4030-01-011093-53-6';
                                ...
                                document.getElementById('editNoRekening').value = noRekening;

                                // Set both the display and the actual value for total
                                document.getElementById('editTotal').value = parseFloat(total)
        ...
        function deleteKwitansi(id) {
            ...
            // Send delete request to API - PERBAIKAN: ganti endpoint
            fetch(`/admin/kwitansi/${id}`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
        ...
        function closeBuatKwitansiModal() {
            ...
            // Reset bank and rekening to default
            document.getElementById('bank').value = 'BRI';
            document.getElementById('noRekening').value = '4030-01-011093-53-6';
