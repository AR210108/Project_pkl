...
<style>
:root { --bg-primary: #f8fafc; --bg-secondary: #ffffff; --bg-alt: #f1f5f9; --text-primary: #1e293b; --text-secondary: #64748b; --border-color: rgba(226, 232, 240, 0.5); --shadow-color: rgba(0, 0, 0, 0.08); --card-bg: linear-gradient(145deg, #ffffff, #f8fafc); --table-hover: #f8fafc; }
.dark { --bg-primary: #0f172a; --bg-secondary: #1e293b; --bg-alt: #334155; --text-primary: #f1f5f9; --text-secondary: #cbd5e1; --border-color: rgba(51, 65, 85, 0.5); --shadow-color: rgba(0, 0, 0, 0.3); --card-bg: linear-gradient(145deg, #1e293b, #334155); --table-hover: #334155; }
body { font-family: 'Roboto', sans-serif; background: linear-gradient(135deg, var(--bg-primary) 0%, var(--bg-alt) 100%); min-height: 100vh; color: var(--text-secondary); }
* { box-sizing: border-box; }
.card { background: var(--card-bg); border-radius: 1.25rem; padding: 2rem; box-shadow: 0 10px 25px var(--shadow-color); border: 1px solid var(--border-color); }
.action-card { cursor: pointer; transition: all 0.3s ease; text-align: center; display: flex; flex-direction: column; align-items: center; justify-content: center; }
.action-card:hover { transform: translateY(-5px); box-shadow: 0 20px 40px var(--shadow-color); }
.action-card .material-icons { font-size: 3.5rem; margin-bottom: 1rem; color: #3b82f6; }
.clock-container { text-align: center; position: relative; overflow: hidden; }
.clock-time { font-size: 3.5rem; font-weight: 700; background: linear-gradient(90deg, #3b82f6, #8b5cf6); -webkit-background-clip: text; background-clip: text; color: transparent; }
...
.pagination-btn.active { background: linear-gradient(90deg, #3b82f6, #8b5cf6); color: white; border-color: transparent; }
.swal2-popup { font-family: 'Roboto', sans-serif; border-radius: 1.25rem; box-shadow: 0 25px 50px -12px var(--shadow-color); background: var(--bg-secondary); border: 1px solid var(--border-color); color: var(--text-primary); }
.swal2-header { background: linear-gradient(135deg, #3b82f6, #8b5cf6); padding: 1.25rem 1.5rem 1rem; margin: 0 !important; border-radius: 1.25rem 1.25rem 0 0; }
...
.uniform-input, .uniform-textarea { border-radius: 0.75rem !important; border: 2px solid var(--border-color) !important; padding: 0.75rem 1rem !important; width: 100% !important; font-size: 0.9rem !important; transition: all 0.3s ease !important; background: var(--bg-alt) !important; font-family: 'Roboto', sans-serif !important; color: var(--text-primary); }
.uniform-input { height: 48px !important; }
.uniform-input:focus, .uniform-textarea:focus { border-color: #3b82f6 !important; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1) !important; background: var(--bg-secondary) !important; outline: none !important; }
...
@media (max-width: 640px) { .swal2-popup { width: 95% !important; max-width: 95% !important; } .clock-time { font-size: 2.5rem; } .action-card { padding: 1.5rem; } .action-card .material-icons { font-size: 2.5rem; } .card { padding: 1.5rem; } }
</style>
</head>
<body>
...
<main class="flex-grow w-full max-w-7xl mx-auto">
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
    <div class="card action-card absensi-btn" data-action="Absen Masuk"><span class="material-icons">login</span><p class="font-semibold" style="color: var(--text-primary);">ABSEN MASUK</p></div>
    <div class="card action-card absensi-btn" data-action="Absen Pulang"><span class="material-icons">logout</span><p class="font-semibold" style="color: var(--text-primary);">ABSEN PULANG</p></div>
    <div class="card action-card absensi-izin-btn" data-action="Izin"><span class="material-icons">event_busy</span><p class="font-semibold" style="color: var(--text-primary);">IZIN</p></div>
    <div class="card action-card absensi-dinas-btn" data-action="Dinas Luar"><span class="material-icons">work_outline</span><p class="font-semibold" style="color: var(--text-primary);">DINAS LUAR</p></div>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="card lg:col-span-2">
      <h3 class="font-bold text-xl mb-4 flex items-center justify-between" style="color: var(--text-primary);">
        <span class="flex items-center"><span class="material-icons text-primary mr-2">history</span>Riwayat Absensi</span>
        <button class="text-sm text-primary hover:underline" id="view-all-btn">Lihat Semua</button>
      </h3>
      <table class="w-full text-left">
        <thead><tr style="color: var(--text-primary); border-bottom: 1px solid var(--border-color);"><th class="pb-3 font-medium">No</th><th class="pb-3 font-medium">Tanggal</th><th class="pb-3 font-medium">Jam Masuk</th><th class="pb-3 font-medium">Jam Pulang</th><th class="pb-3 font-medium">Status</th></tr></thead>
        <tbody id="history-tbody" style="color: var(--text-secondary);"></tbody>
      </table>
      <div id="empty-state" class="empty-state" style="display: none;"><span class="material-icons">assignment_late</span><h4>Belum Ada Riwayat Absensi</h4><p>Anda belum memiliki riwayat absensi. Silakan lakukan absen terlebih dahulu.</p></div>
    ...
    <!-- Status Absensi -->
    <div class="card">
      <h3 class="font-bold text-xl mb-4 flex items-center" style="color: var(--text-primary);"><span class="material-icons text-primary mr-2">assignment</span>Status Absensi</h3>
      <div class="space-y-3">
        <div class="flex items-center gap-3"><span class="material-icons text-primary">login</span><div><p class="font-medium" style="color: var(--text-primary);">Absen Masuk</p><p class="text-sm" id="today-checkin" style="color: var(--text-secondary);">-</p></div></div>
        <div class="flex items-center gap-3"><span class="material-icons text-primary">logout</span><div><p class="font-medium" style="color: var(--text-primary);">Absen Pulang</p><p class="text-sm" id="today-checkout" style="color: var(--text-secondary);">-</p></div></div>
        <div class="flex items-center gap-3"><span class="material-icons text-success">check_circle</span><div><p class="font-medium" style="color: var(--text-primary);">Status Hari Ini</p><p class="font-medium text-success" id="today-status">Belum Absen</p></div></div>
      </div>
    </div>
  </div>
</main>
<footer class="w-full max-w-7xl mx-auto mt-8 text-center text-xs sm:text-sm py-4" style="color: var(--text-secondary);"><p>Copyright ©2025 by digicity.id</p></footer>
</div>
