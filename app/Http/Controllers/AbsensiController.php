 ...
 */
    public function kelolaAbsenManajer()
    {
        try {
            ...
            $selectedDivision = request('division', 'semua');
            $statusFilter = request('status', 'semua');

            // 2. QUERY DATA ABSENSI HARIAN (dari attendance_records)
            $absensiQuery = DB::table('attendance_records')
                ->leftJoin('users', 'attendance_records.employee_id', '=', 'users.employee_id')
                ->select(
                    'attendance_records.*',
                    'users.name',
                    'users.divisi'
            ...
            if ($selectedDivision && $selectedDivision !== 'semua') {
                $absensiQuery->where('users.divisi', $selectedDivision);
            }

            // Filter berdasarkan status
            if ($statusFilter && $statusFilter !== 'semua') {
                if ($statusFilter === 'hadir') {
                    $absensiQuery->whereIn('attendance_records.status', ['Tepat Waktu', 'Terlambat']);
                } elseif ($statusFilter === 'terlambat') {
                    $absensiQuery->where('attendance_records.status', 'Terlambat');
                } elseif (in_array($statusFilter, ['izin', 'cuti', 'sakit', 'dinas'])) {
                    // Untuk izin/cuti/sakit/dinas, ambil dari tabel attendances
                    // Kita akan handle di bagian lain
                }
            }

            $absensiData = $absensiQuery->orderBy('attendance_records.date', 'desc')
                ->get();

            // 3. QUERY DATA KETIDAKHADIRAN (dari attendances)
            $ketidakhadiranQuery = DB::table('attendances')
                ...
                ->whereNotNull('attendances.jenis_ketidakhadiran');

            // Filter divisi untuk ketidakhadiran
            if ($selectedDivision && $selectedDivision !== 'semua') {
                $ketidakhadiranQuery->where('users.divisi', $selectedDivision);
            }

            // Filter status untuk ketidakhadiran
            if ($statusFilter && $statusFilter !== 'semua') {
                if ($statusFilter === 'pending') {
                    $ketidakhadiranQuery->where('attendances.approval_status', 'pending');
                } elseif ($statusFilter === 'approved') {
                    $ketidakhadiranQuery->where('attendances.approval_status', 'approved');
                } elseif ($statusFilter === 'rejected') {
                    $ketidakhadiranQuery->where('attendances.approval_status', 'rejected');
                } elseif ($statusFilter === 'izin') {
                    $ketidakhadiranQuery->where('attendances.jenis_ketidakhadiran', 'izin');
                } elseif ($statusFilter === 'cuti') {
                    $ketidakhadiranQuery->where('attendances.jenis_ketidakhadiran', 'cuti');
                } elseif ($statusFilter === 'sakit') {
                    $ketidakhadiranQuery->where('attendances.jenis_ketidakhadiran', 'sakit');
                } elseif ($statusFilter === 'dinas') {
                    $ketidakhadiranQuery->where('attendances.jenis_ketidakhadiran', 'dinas-luar');
                }
            }

            $ketidakhadiranData = $ketidakhadiranQuery->orderBy('attendances.tanggal', 'desc')
                ->get();

            // 4. GABUNGKAN DATA UNTUK DITAMPILKAN DI TABEL
            $formattedAbsensi = collect();
            
            // Data dari attendance_records (kehadiran)
            foreach ($absensiData as $absensi) {
                ...
                $formattedAbsensi->push([
                    'id' => $absensi->id,
                    'type' => 'kehadiran',
                    'user_name' => $absensi->name ?? 'Tidak Diketahui',
                    'divisi' => $absensi->divisi ?? '-',
                    'tanggal' => $absensi->date,
                    'jam_masuk' => $absensi->check_in ? Carbon::parse($absensi->check_in)->format('H:i') : '-',
                    'jam_pulang' => $absensi->check_out ? Carbon::parse($absensi->check_out)->format('H:i') : '-',
                    'status_kehadiran' => $status,
                    'status_class' => $statusClass,
                    'approval_status' => 'approved', // Data attendance_records sudah approved
                    'keterangan' => $absensi->reason,
                ]);
            }
            
            // Data dari attendances (ketidakhadiran)
            foreach ($ketidakhadiranData as $ketidakhadiran) {
                $jenis = $ketidakhadiran->jenis_ketidakhadiran;
                $statusClass = $this->getStatusClass($jenis);
                
                $formattedAbsensi->push([
                    'id' => $ketidakhadiran->id,
                    'type' => 'ketidakhadiran',
                    'user_name' => $ketidakhadiran->name ?? 'Tidak Diketahui',
                    'divisi' => $ketidakhadiran->divisi ?? '-',
                    'tanggal' => $ketidakhadiran->tanggal,
                    'jam_masuk' => '-',
                    'jam_pulang' => '-',
                    'status_kehadiran' => ucfirst($jenis),
                    'status_class' => $statusClass,
                    'approval_status' => $ketidakhadiran->approval_status,
                    'keterangan' => $ketidakhadiran->keterangan,
                    'tanggal_akhir' => $ketidakhadiran->tanggal_akhir,
                ]);
            }
            
            // Urutkan berdasarkan tanggal (terbaru dulu)
            $formattedAbsensi = $formattedAbsensi->sortByDesc('tanggal')->values();

            // 5. HITUNG STATISTIK
            // Data dari attendance_records
            $totalTepatWaktu = DB::table('attendance_records')
                ->leftJoin('users', 'attendance_records.employee_id', '=', 'users.employee_id')
                ->whereBetween('attendance_records.date', [$startDate, $endDate])
                ->where('attendance_records.status', 'Tepat Waktu')
                ->when($selectedDivision && $selectedDivision !== 'semua', function($query) use ($selectedDivision) {
                    return $query->where('users.divisi', $selectedDivision);
                })
                ->count();
            ...
            $totalTidakMasuk = DB::table('attendance_records')
                ->leftJoin('users', 'attendance_records.employee_id', '=', 'users.employee_id')
                ->whereBetween('attendance_records.date', [$startDate, $endDate])
                ->where('attendance_records.status', 'Tidak Masuk')
                ->when($selectedDivision && $selectedDivision !== 'semua', function($query) use ($selectedDivision) {
                    return $query->where('users.divisi', $selectedDivision);
                })
                ->count();
            ...
            // Data dari attendances (ketidakhadiran)
            $totalIzin = DB::table('attendances')
                ->leftJoin('users', 'attendances.user_id', '=', 'users.id')
                ->whereBetween('attendances.tanggal', [$startDate, $endDate])
                ->where('attendances.jenis_ketidakhadiran', 'izin')
            ...
            $totalCuti = DB::table('attendances')
                ->leftJoin('users', 'attendances.user_id', '=', 'users.id')
                ->whereBetween('attendances.tanggal', [$startDate, $endDate])
                ->where('attendances.jenis_ketidakhadiran', 'cuti')
                ->where('attendances.approval_status', 'approved')
                ->when($selectedDivision && $selectedDivision !== 'semua', function($query) use ($selectedDivision) {
                    return $query->where('users.divisi', $selectedDivision);
                })
            ...
            $totalSakit = DB::table('attendances')
                ->leftJoin('users', 'attendances.user_id', '=', 'users.id')
                ->whereBetween('attendances.tanggal', [$startDate, $endDate])
                ->where('attendances.jenis_ketidakhadiran', 'sakit')
                ->where('attendances.approval_status', 'approved')
                ->when($selectedDivision && $selectedDivision !== 'semua', function($query) use ($selectedDivision) {
                    return $query->where('users.divisi', $selectedDivision);
                })
            ...
            $totalDinasLuar = DB::table('attendances')
                ->leftJoin('users', 'attendances.user_id', '=', 'users.id')
                ->whereBetween('attendances.tanggal', [$startDate, $endDate])
                ->where('attendances.jenis_ketidakhadiran', 'dinas-luar')
                ->where('attendances.approval_status', 'approved')
                ->when($selectedDivision && $selectedDivision !== 'semua', function($query) use ($selectedDivision) {
                    return $query->where('users.divisi', $selectedDivision);
                })
            ...
            $totalPending = DB::table('attendances')
                ->leftJoin('users', 'attendances.user_id', '=', 'users.id')
                ->whereBetween('attendances.tanggal', [$startDate, $endDate])
            ...
            $totalApproved = DB::table('attendances')
                ->leftJoin('users', 'attendances.user_id', '=', 'users.id')
                ->whereBetween('attendances.tanggal', [$startDate, $endDate])
                ->where('attendances.approval_status', 'approved')
                ->when($selectedDivision && $selectedDivision !== 'semua', function($query) use ($selectedDivision) {
            ...
            $totalRejected = DB::table('attendances')
                ->leftJoin('users', 'attendances.user_id', '=', 'users.id')
                ->whereBetween('attendances.tanggal', [$startDate, $endDate])
                ->where('attendances.approval_status', 'rejected')
                ->when($selectedDivision && $selectedDivision !== 'semua', function($query) use ($selectedDivision) {
                    return $query->where('users.divisi', $selectedDivision);
                })
                ->count();
    ...
    private function getStatusClass($status)
    {
        ...
        if (strpos($status, 'tepat') !== false || strpos($status, 'on-time') !== false) {
            return 'status-hadir';
        } elseif (strpos($status, 'terlambat') !== false || strpos($status, 'late') !== false) {
            return 'status-terlambat';
        } elseif (strpos($status, 'tidak masuk') !== false || strpos($status, 'no-show') !== false) {
...
private function getStatusKehadiran($absen)
{
    if ($absen->jenis_ketidakhadiran) {
        switch ($absen->jenis_ketidakhadiran) {
            ...
            case 'cuti':
                return ['label' => 'Cuti', 'class' => 'status-cuti'];
            case 'sakit':
                return ['label' => 'Sakit', 'class' => 'status-sakit'];
            case 'dinas-luar':
                return ['label' => 'Dinas Luar', 'class' => 'status-dinas'];
            default:
    ...
    return ['label' => 'Tidak Hadir', 'class' => 'bg-red-100 text-red-800'];
}

    /**
     * Action untuk approval/rejection (jika GM memiliki hak)
     */
    ...
    /**
 * Menampilkan halaman rekap absensi untuk General Manager
 * 
 * @return \Illuminate\View\View
 */
        ...
        try {
            ...
            $response = [
                ...
                'data' => $attendance ? [
                    ...
                    'jenis_ketidakhadiran_label' => $attendance->jenis_ketidakhadiran ? $this->getJenisKetidakhadiranLabel($attendance->jenis_ketidakhadiran) : null,
